...
int function_for_strlen(char* p1){
    int counter = 0;
    while(p1[counter]){
        counter++;
    }
...
int function_for_strcmp(char* p1, char* p2){
    int len_1 = function_for_strlen(p1);
    int len_2 = function_for_strlen(p2);
    if( len_1 != len_2){
        return 1;
...
void main_work(int argc, char** argv){
    ...
    for(int i = 1; i < argc; i++){
        if((function_for_strcmp(argv[i], "-c")) == 0){
            secret_code = my_strcpy(secret_code, argv[i+1]);
        }else if((function_for_strcmp(argv[i], "-t")) == 0){
            attemps = str_to_int(argv[i+1]);
        }
    }
    if(secret_code[0] == 0){
...
void loadingAnimation() {
    char animation[] = "|/-\\";
    int animationLength = sizeof(animation) - 1;
    int delayMs = 100;

    int i;
    for (i = 0; i < 10; ++i) {
        printf("Loading... %c\r", animation[i % animationLength]);
        fflush(stdout);
        usleep(delayMs * 1000);
    }
}
