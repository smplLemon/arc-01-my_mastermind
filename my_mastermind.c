...
void checking_function(char* users_guess, char* secret_code, int* right, int* wrong)
{
    ...
    int users_guess_users_guess[10] = {0};
    int c1[10] = {0};
    for (int i = 0; i < the_length_of_secret_code; i++){
        if (users_guess[i] == secret_code[i]){
            (*right)++;
        } else {
            users_guess_users_guess[users_guess[i] - '0']++;
            c1[secret_code[i] - '0']++;
        }
    }
    while(i<10){
        if (c1[i] < users_guess_users_guess[i]){
            *wrong += c1[i];
        } else {
            *wrong += users_guess_users_guess[i];
        }
        i++;
    }
}
...
int input_code_check_function(char* input_code)
{
    int i = 0;
...
int main(int argc, char** argv)
{
    ...
    while (rounds < maksimal_urinishlar){
        ...
        if (!input_code_check_function(users_guess)){
            printf("Wrong input!\n");
            continue;
        }
